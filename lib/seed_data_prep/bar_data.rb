...
bars = []
size.times do |x|
  bars << Hash[:name,data["businesses"][x]["name"],:latitude,data["businesses"][x]["location"]["coordinate"]["latitude"],:longitude,data["businesses"][x]["location"]["coordinate"]["longitude"]]
end
...
CSV.open("bar_data_clean.csv", "wb") do |csv|
  bars.each do |bars|
    csv << [bars[:name], bars[:latitude],bars[:longitude]]
  end
end